...
public class NameEdit implements GuiAction, GuiChatAction {
	...
	public NameEdit(Player player, GUI gui, EditAction editAction, int place){
		this.gui = gui;
		this.player = player;
	...
	public void execute(Player player, String value){
		...
		GuiButton nameButton = GuiButton.ButtonBuilder.getInstance().
			setItemStack(GuiItem.valueOf(player, languageConfig,"Gui.EditItem.Action.Name", "{name}", value)).
			setGuiAction(this).
			build();
		gui.setButton(nameButton, 3, 2);

		editAction.name = value;
