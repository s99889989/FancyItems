...
public class ActionEdit implements GuiAction, GuiChatAction {
	...
	public ActionEdit(Player player, GUI gui, EditAction editAction, int place){
		this.gui = gui;
		this.player = player;
	...
	public void execute(Player player, String value){
		...
		GuiButton nameButton = GuiButton.ButtonBuilder.getInstance().
			setItemStack(GuiItem.valueOf(player, languageConfig,"Gui.EditItem.Action.Action", "{action}", value)).
			setGuiAction(this).
			build();
		gui.setButton(nameButton, 3, 3);

		editAction.actionKey = value;
